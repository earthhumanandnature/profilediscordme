...
.then(user => {
  ...
  document.getElementById('username').innerHTML = `${displayName} <span>#${user.discriminator || '0000'}</span>`;

  // Banner (đã sửa lỗi ngoặc thừa)
  if (user.banner) {
...
// Cho phép phát nhạc khi người dùng tương tác lần đầu
document.body.addEventListener('click', () => audio.play().catch(() => {}), { once: true });

// ==================== HIỆU ỨNG NGHIÊNG CARD 3D ====================
const applyTilt = (e) => {
...
// Gắn sự kiện chỉ khi card đã hiện (sau khi login thành công)
const waitForCard = () => {
